...
export const deleteWorkout = async (req, res, next) => {
  try {
    ...
    console.log('Delete request - User ID:', userId, 'Workout ID:', workoutId);
    
    const workout = await Workout.findOneAndDelete({ _id: workoutId, user: userId });
    console.log('Delete result:', workout);
...
export const duplicateWorkout = async (req, res, next) => {
  try {
    ...
    const originalWorkout = await Workout.findOne({ _id: workoutId, user: userId });
    if (!originalWorkout) {
      return next(createError(404, "Workout not found"));
    }
    ...
    const duplicatedWorkout = new Workout({
      ...
      sets: originalWorkout.sets,
      reps: originalWorkout.reps,
      weight: originalWorkout.weight,
...
export const importSmartwatchData = async (req, res, next) => {
  try {
    const userId = req.user?.id;
    const user = await User.findById(userId);
    if (!user) {
    ...
    // Simulate smartwatch data import
    const smartwatchWorkouts = [
      {
        category: "Cardio",
    ...
    await Workout.insertMany(smartwatchWorkouts);

    return res.status(201).json({
      message: "Smartwatch data imported successfully",
      workouts: smartwatchWorkouts,
    });
  } catch (err) {
    next(err);
  }
